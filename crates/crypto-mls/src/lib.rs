...
mod tests {
    ...
    fn bootstraps_direct_session_with_two_members() {
        ...
        let group = manager
            .bootstrap_direct(&ConversationId("dm:rsaga:builder".into()), direct_members())
    ...
    fn seal_and_open_roundtrip_message() {
        let mut manager = SkeletonSecureSessionManager::new();
        let conversation_id = ConversationId("dm:rsaga:builder".into());
        manager
    ...
    fn snapshot_roundtrip_preserves_groups() {
        ...
        let group = restored
            .group_state(&ConversationId("dm:rsaga:builder".into()))
