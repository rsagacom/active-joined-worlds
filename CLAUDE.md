...
处理 IM 开发、审查、验收、前端修复或后台改造时，按顺序读取：
1. `.claude/project-state.md`（SDLC 阶段状态）
2. `docs/DS_V4_PRO_CC_HANDOFF_20260525.md`
3. `docs/ACTIVE_WORK_QUEUE.md`
4. `docs/IMPLEMENTATION_PHASES.md`
5. `docs/PRODUCT_CHARTER.md`

## 项目不可偏离方向

- 单城中心化 IM 先真实可用，再考虑跨城去中心化。
- Gateway 是唯一合同真源；H5 只做渲染，不制造本地 canonical state。
- H5 三层结构：底层像素场景、中层透明热点、顶层微信式左右气泡。
- 大文件、构建包、导出包默认放 `/Volumes/AJW-Data`。
