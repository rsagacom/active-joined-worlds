...
fn no_args_reports_missing_command() {
    ...
    assert!(stderr.contains("missing command") || stderr.contains("command"),
        "should report missing command");
}

#[test]
fn send_help_shows_usage() {
    let output = cli_binary().args(["send", "--help"]).output().expect("run cli");
    // May succeed with help or fail with bad args; either is OK
    let combined = format!("{}{}", String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
    assert!(combined.contains("send") || combined.contains("--to") || combined.contains("from"),
...
fn send_missing_to_argument_reports_error() {
    ...
    assert!(combined.contains("--to") || combined.contains("to"),
